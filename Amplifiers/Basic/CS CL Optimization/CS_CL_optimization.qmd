---
title: "Optimization of CS SC Amplifier"
subtitle: "Version 1"
...
In this notebook, we want to minimize the bias current of the common-source (CS) closed-loop (CL) amplifier of @fig-CS_CL_amplifier_schematic for achieving a given gain and bandwidth.
...
The amplifier bandwidth is given by $\omega_c = \omega_p \cong \beta \cdot G_m/C_{out}$ where $C_{out}$ is the total capacitance seen at the output
\begin{equation}
...
#saveFigures(savePath, 'CS_opt_ib_vs_IC_no_VS')
plt.show()
```

From @fig-ib_vs_ic, we clearly see that there is a minimum current for a given value of parameter $\Theta$. We can find the optimum inversion coefficient $IC_{opt}$ which is given by
\begin{align}
...
plt.text(0.7, 0.95, 'No VS ($\\lambda_c = 0$)', ha='left', va='top', size=9, transform=plt.gca().transAxes,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
#saveFigures(savePath, 'CS_opt_AR_vs_IC_no_VS')
plt.show()
```

We see from @fig-woverl_vs_ic that the transistor width increases first as $1/\sqrt{IC}$ in strong inversion and then as $1/IC$ in weak inversion making the transistor quickly very large until $IC$ reaches $IC_{lim}$ where the width becomes infinity. The dots correspond to the $AR$ obtained for $IC_{opt}$.
...
Adc=10**(AdcdB/20)
wc=2*pi*BW
table = [["DC gain","$A_{{dc}}$",f'{AdcdB:.0f}',"$dB$"],
...
#| tbl-cap: CS SC amplifier optimum parameters.
...
textstr1 = '\n'.join((
    r'$A_{0,ideal} =$'+ f'{A0ide:.1f}',
    r'$A_{dc} =$'+ f'{Adcthe:.1f}',
    r'$A_0 =$'+ f'{A0the:.1f}',
    r'$f_c =$'+ f'{fcide/1e3:.0f} kHz',
    r'$f_c =$'+ f'{fcthe/1e3:.0f} kHz',
    r'$GBW =$'+ f'{GBWide/1e6:.1f} MHz',
    r'$GBW =$'+ f'{GBWthe/1e6:.1f} MHz'))
...
textstr2 = '\n'.join((
    r'$L =$'+ f'{L/1e-6:.0f} $\\mu m$',
    r'$C_L =$'+ f'{CL/1e-12:.0f} pF',
    r'$C_S =$'+ f'{CS/1e-12:.0f} pF',
    r'$C_F =$'+ f'{CF/1e-15:.3f} fF',
    r'$C_{{GD}} =$'+ f'{CGD/1e-15:.3f} fF',
    r'$C_{{in}} =$'+ f'{Cin/1e-15:.3f} fF',
    f'$\\beta =$ {beta:.3f}',
    r'$IC_{opt} =$'+ f'{ICopt:.2f}',
    r'$I_{b,opt} =$'+ f'{Ibopt/1e-9:.0f} nA',
    r'$W_{opt} =$'+ f'{Wopt/1e-6:.2f} $\\mu m$'))
