...
\end{equation}
where
\begin{equation}\label{eqn:etaththe}
  \eta_{th} =\frac{\gamma_{n2}}{\gamma_{n1}} \cdot \frac{G_{m2}}{G_{m1}}
\end{equation}
represents the contribution to the input-referred thermal noise of the current mirror M~2a~-M~2b~ relative to that of the differential pair M~1a~-M~1b~. In case $G_{m1} \gg G_{m2}$ then $\eta_{th} \ll 1$ and the thermal noise is dominated by the input differential pair. Eqn. \eqref{eqn:etaththe} can then be simplified to
\begin{equation}
...
etaflthe=1
etaththe=0
WL1=rhon*Gm1/(fkspec*gamman1)*(1+etaflthe)/(1+etaththe)
W1=sqrt(WL1*WoverL1)
L1=sqrt(WL1/WoverL1)
W1=round(1E8*W1)/1E8
L1=round(1E8*L1)/1E8
...
\end{equation}
We will assume that $\eta_{th} \ll 1$ and set $\eta_{fl} = 1$. We can then deduce the gate area of M~1a~-M~1b~ as
\begin{equation}
  W_1\,L_1 = \frac{\rho_n\,G_{m1}}{f_k\,\gamma_{n1}} \cdot \frac{1+\eta_{fl}}{1+\eta_{th}}.
\end{equation}
This leads to $W_1\,L_1 =$ `{python} f'{WL1/1e-12:.2f}'` $\mu m^2$ and $W_1 =$ `{python} f'{W1/1e-6:.2f}'` $\mu m$ and $L_1 =$ `{python} f'{L1/1e-6:.2f}'` $\mu m$.
...
\begin{equation}
  C_{GS2} \cong W_2\,L_2\,C_{ox} \cdot c_{gsi} + C_{GSop} \cdot W_2,
\end{equation}
...
etaththe=gamman2/gamman1*Gm2/Gm1
etaflthe=(Gm2/Gm1)**2*rhop/rhon*W1L1/(W2L2)
fkthe=rhon*Gm1/(W1*L1*gamman1)*(1+etaflthe)/(1+etaththe)
```

We can check the contributions of M~2a~-M~2b~ to the thermal and flicker noise $\eta_{th} =$ `{python} f'{etaththe:.3f}'` and $\eta_{fl} =$ `{python} f'{etaflthe:.3f}'`. The resulting corner frequency is then given by $f_k =$ `{python} f'{fkthe/1e3:.3f}'` $kHz$ which is higher than the specification $f_k =$ `{python} f'{fkspec/1e3:.0f}'` $kHz$ because of the larger $\eta_{fl} =$ compared to the initial value. We can slightly increase the gate area $W_1\,L_1$ of M~1a~-M~1b~.
...
With $W_1\,L_1 =$ `{python} f'{WL1/1e-12:.3f}'` $\mu m^2$, the resulting corner frequency is  $f_k =$ `{python} f'{fkthe/1e3:.3f}'` $kHz$, which is now lower than the specification $f_k =$ `{python} f'{fkspec/1e3:.0f}'` $kHz$.

The new values of $W_1$ and L_1$ are given by $W_1 =$ `{python} f'{W1/1e-6:.2f}'` $\mu m$ and $L_1 =$ `{python} f'{L1/1e-6:.2f}'` $\mu m$.
...
In order to maximize the DC gain we will choose $L_3 =$ `{python} f'{L3/1e-9:.0f}'` $nm$. Keeping the same $W/L$ results in $W_3 =$ `{python} f'{W3/1e-6:.2f}'` $\mu m$. The DC gain is now $A_{dc} =$ `{python} f'{AdcestdB:.3f}'` dB.
...
\end{equation}
Setting the source-to-drain voltage of M~4a~-M~4b~ to $V_{SD4}  =$ `{python} f'{VSD4/1e-3:.0f}'` $mV$, with $V_{SG4} =$ `{python} f'{VSG4/1e-3:.0f}'` $mV$ and $V_{SG2} =$ `{python} f'{VSG2/1e-3:.0f}'` $mV$, we get $V_{b2,min}  =$ `{python} f'{Vb2min:.3f}'` $V$.

```{python}
VDS3=0.2
#VGS3=VT0n+n0n*UT*vps_ic(IC3)
VGS3=VT0n
Vb1max=VGS3-VDS3-VSG2+VDD
VDS1=0.2
Vic=0.9
VGS1=VT0n
Vb1min=VGS3+VDS1-VGS1+Vic
#Vic1=VGS1-VDS1-VGS3+Vb1
```

Similarly, we can calculate the maximum bias voltage $V_{b1,max}$ allocating enough $V_{DS}$ voltage for M~3a~-M~3b~ not to degrade its output conductance. This leads to
\begin{equation}
  V_{b1,max} = V_{GS3} - V_{DS3} - V_{SG2} + V_{DD}.
\end{equation}
Allocating $V_{DS3} =$ `{python} f'{VDS3/1e-3:.0f}'` $mV$, with $V_{GS3}  =$ `{python} f'{VGS3/1e-3:.0f}'` $mV$ and $V_{SG2}  =$ `{python} f'{VSG2/1e-3:.0f}'` $mV$, we get $V_{b1,max}  =$ `{python} f'{Vb1max:.3f}'` $V$.

We also want to make sure that the $V_{DS}$ voltage of M~1a~-M~1b~ is large enough not to degrade its output conductance. This sets a minimum value for $V_{b1}$ given by
\begin{equation}
  V_{b1,min} = V_{GS3} + V_{DS1} - V_{GS1} + V_{ic}.
\end{equation}
For $V_{ic} =$ `{python} f'{Vic:.1f}'` $V$ and $V_{DS1} =$ `{python} f'{VDS1/1e-3:.0f}'` $mV$, with $V_{GS1} =$ `{python} f'{VGS1/1e-3:.0f}'` $mV$, we get $V_{b1,min}  =$ `{python} f'{Vb1min:.3f}'` $V$
...
::: {.callout-note}
Setting $V_{b1}$ and $V_{b2}$ is tricky and requires a bit of fine tuning. After checking by simulation, we finally choose $V_{b1} =$ `{python} f'{Vb1:.3f}'` $V$ and $V_{b2}  =$ `{python} f'{Vb2:.3f}'` $V$.
:::
...
{
    "Name": ["AdcdB","GBWspec","CL","Vosmax","PMdeg","fk"],
    "Value": [AdcdB,GBWspec,CL,Vosmax,PMdeg,fkspec]
})
...
etaththe=gamman2/gamman1*Gm2/Gm1
gammanotathe=2*gamman1*(1+etaththe)
...
W2L2=W2eff*L2eff
etaflthe=(Gm2/Gm1)**2*rhop/rhon*W1L1/(W2L2)
Rnfl1Hz=2*rhon/(W1L1)*(1+etaflthe)
Sfl1Hz=4*kT*Rnfl1Hz
Sninfl1HzdBv=10*log(Sfl1Hz)
#fkthe=2*rhon/(W1*L1*Rninththe)*(1+etaflthe)
...
table = [["$(G_{m1}/G_{m2})^2$",(Gm1/Gm2)**2,"-",""],
         ...
         ["$(1+\\eta_{{fl}})/(1+\\eta_{{th}})$",(1+etaflthe)/(1+etaththe),"-",""],
...
df_mos=pd.read_table(ngspiceDataFile, sep=r'\s+', skiprows=[2,4,6,8,10,12,14,16,18], dtype=np.float64, engine='python')
...
VDsat=VDSsat2b+VDSsat4b+VDSsat3b+VDSsat1b+VDSsat5b
Voutswing=VDD-VDsat
...
feature_list=["gamman","Rn","Vnth","Vnfl at 1Hz"]
...
Markdown(tabulate(
  table,
  headers=["Transistor","$\\gamma_n$","$R_{n,th}\\;[k\\Omega]$","$V_{n,th}\\;[nV/\\sqrt{Hz}]$","$V_{n,fl}$ at 1Hz $[\\mu V/\\sqrt{Hz}]$"],
  numalign="center",
  stralign="center",
  colalign=("center","center","center","center","center"),
  floatfmt=(".3f",".2f",".1f",".1f",".1f")
))
```
...
Markdown(tabulate(
  table,
  headers=["Trans.","Type","Funct.","$V_G\\;[V]$","$V_S\\;[V]$","$V_D\\;[V]$","$V_{DS}\\;[mV]$","$V_{Dsat}\\;[mV]$","Reg.","Sat."],
  numalign="center",
  stralign="center",
...
plt.plot([Vidmin/1e-6,0],[Voutq0,Voutq0],'k--', linewidth=lw)
plt.plot([0,0],[0.8,Voutq0],'k--', linewidth=lw)
plt.plot([Vidmin/1e-6,Vos/1e-6],[Voutq,Voutq],'k--', linewidth=lw)
plt.plot([Vos/1e-6,Vos/1e-6],[0.8,Voutq],'k--', linewidth=lw)
plt.plot(Vidsim/1e-6,Voutsim,'r')
plt.xlim(Vidmin/1e-6,Vidmax/1e-6)
...
## Open-loop gain
After having checked the operating point information and making sure that the OTA output is not saturated by extracting the required offset voltage for bringing the output operating point to the desired value $V_{outq} =$ `{python} f'{Voutq:.3f}'` $V$, we can now perform the AC simulation. The open-loop transfer function is shown in @fig-ng_open_loop_gain.
...
for k in range(0,Npts):
  ...
  VnoutM3aw[k]=df_noise.iloc[Nvar*k+16,0]
  VnoutM3b[k]=df_noise.iloc[Nvar*k+17,0]
  VnoutM3bf[k]=df_noise.iloc[Nvar*k+18,0]
  VnoutM3bw[k]=df_noise.iloc[Nvar*k+19,0]
  VnoutM4a[k]=df_noise.iloc[Nvar*k+20,0]
  VnoutM4af[k]=df_noise.iloc[Nvar*k+21,0]
  VnoutM4aw[k]=df_noise.iloc[Nvar*k+22,0]
  VnoutM4b[k]=df_noise.iloc[Nvar*k+23,0]
  VnoutM4bf[k]=df_noise.iloc[Nvar*k+24,0]
  VnoutM4bw[k]=df_noise.iloc[Nvar*k+25,0]
  VnoutM5a[k]=df_noise.iloc[Nvar*k+26,0]
  VnoutM5af[k]=df_noise.iloc[Nvar*k+27,0]
  VnoutM5aw[k]=df_noise.iloc[Nvar*k+28,0]
  VnoutM5b[k]=df_noise.iloc[Nvar*k+29,0]
  VnoutM5bf[k]=df_noise.iloc[Nvar*k+30,0]
  VnoutM5bw[k]=df_noise.iloc[Nvar*k+31,0]
...
@fig-ng_white_noise_contributions shows that the simulated total input-referred white noise PSD perfectly matches the theoretical prediction. @fig-ng_white_noise_contributions also shows the contributions of M~1a~-M~1b~, M~2a~-M~2b~, M~3a~-M~3b~, M~4a~-M~4b~ and M~5a~-M~5b~ to the input-referred white noise PSD. We can oberve that the white noise is dominated by the contribution of M~1a~-M~1b~. This is confirmed by the value of the simulated $\eta_{th} =$ `{python} f'{etathsim:.3f}'` which is very close to the theoretical value $\eta_{th} =$ `{python} f'{etaththe:.3f}'`. The contribution of M~2a~-M~2b~ is `{python} f'{10*log(SninM1wsim/SninM2wsim):.1f}'` $dB$ lower than the contribution of M~1a~-M~1b~. We also see that the contributions of the cascode transistors M~3a~-M~3b~ and M~4a~-M~4b~ are negligible. The contribution of M~5a~-M~5b~ is also negligible since it is a common mode contribution which is cancelled at the output. Finally, the simulated OTA thermal noise excess factor $\gamma_{n,ota} =$  `{python} f'{gammanotasim:.3f}'` is slightly smaller than the predicted value $\gamma_{n,ota} =$  `{python} f'{gammanotathe:.3f}'`.
...
Similarly to the white noise, @fig-ng_flicker_noise_contributions shows that the total simulated input-referred flicker noise PSD perfectly matches the theoretical prediction. We can observe that the input-referred flicker noise is dominated by the contribution of M~2a~-M~2b~ which is `{python} f'{10*log(SninM2f1Hzsim/SninM1f1Hzsim):.3f}'` $dB$ higher than the contribution of M~1a~-M~1b~. This consistent with the value of $\eta_{fl}=$ `{python} f'{etaflsim:.3f}'` (`{python} f'{10*log(etaflsim):.3f}'` in $dB$) which is close to the theoretical value $\eta_{fl}=$ `{python} f'{etaflthe:.3f}'`. We see that M~2a~-M~2b~ contribute about the same than M~1a~-M~1b~ and that the contributions of the cascode transistors M~3a~-M~3b~ and M~4a~-M~4b~ are negligible. The contributions of the cascode transistors is negligible. Note that the contribution of M~5a~-M~5b~ is not shown because it is completely negligible.
...
if newSim:
  paramstr = '\n'.join((
  f'.param VDD={VDD:.1f} Vic={Vic:.3f} Vb1={Vb1:.1f} Vb2={Vb2:.1f} Ib={Ib/1e-9:.0f}n',
  f'.param CL={CL/1e-12:.0f}p Rb={Rb/1e3:.0f}k',
  f'.param W1={W1/1e-6:.2f}u L1={L1/1e-6:.2f}u',
  f'.param W2={W2/1e-6:.2f}u L2={L2/1e-6:.2f}u',
  f'.param W3={W3/1e-6:.2f}u L3={L3/1e-6:.2f}u',
